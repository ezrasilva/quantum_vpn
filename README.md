...
| HKDF-SHA256 | < 0.1 ms |
| Rekeying IPsec (completo) | 50-200 ms |
| Throughput ESP AES-256-GCM | ~1-5 Gbps (CPU-bound) |

### Escalabilidade

- Implementação atual: 2 túneis simultâneos (Alice-Bob, Carol-Dave)
- Capacidade teórica: Centenas de túneis por controlador
- Gargalo: Operações de socket VICI em nós VPN e CPU para AES-GCM
- Otimização: Hardware AES-NI reduz overhead de cifragem ESP
...
#    Solução: adicionar cap_add: [NET_ADMIN] no docker-compose.yml
# 2. Conflitos de porta 5000
#    Solução: mudar mapeamento de portas
# 3. Biblioteca liboqs ausente
#    Solução: recompilar imagem Docker
```
...
docker exec orchestrator ping 192.168.100.10

# Verificar se agente está ouvindo
docker exec alice netstat -tlnp | grep 5000

# Testar endpoint de saúde do agente
docker exec orchestrator curl http://192.168.100.10:5000/health

# Verificar assinatura ML-DSA-65
# Se retornar 403 → problema de validação de assinatura
```
...
# Ver logs strongSwan detalhados
docker exec alice tail -f /var/log/charon.log
...
# Trigger manual de rekey
docker exec alice swanctl --rekey --ike alice-bob
```

### Erros de QKD

```bash
# Verificar logs de cliente QKD
docker logs orchestrator | grep -i qkd

# Testar conectividade KME (exemplo Alice)
curl --cert scripts/certs/sae-1.crt --key scripts/certs/sae-1.key \
     --cacert scripts/certs/account-*-ca-*.crt \
     https://kme-1.acct-2992.etsi-qkd-api.qukaydee.com/api/v1/keys/...

# Fallback para PQC puro (esperado se QKD falhar)
```
...
# Validação de assinatura ML-DSA-65
...
4. Gerar certificados QKD (se aplicável): `sae-5.crt`, `sae-5.key`
5. Recompilar e reiniciar: `docker-compose up -d --build`
